...
def parse_cli():
	...
	return args.file, args.likelihood, args.min_percentile, args.max_percentile, args.radius_max_percentile, args.radius_min_percentile, args.eyeblink, args.n, args.window
...
def main():
	...
	file, likelihood, min_percentile, max_percentile, radius_max_percentile, radius_min_percentile, eyeblink, n, window = parse_cli()
	...
	while current < len(frame.index):
		...
		x0_max_percentile = current_frame.x0.quantile(max_percentile)
		x0_min_percentile = current_frame.x0.quantile(min_percentile)
		y0_max_percentile = current_frame.y0.quantile(max_percentile)
		y0_min_percentile = current_frame.y0.quantile(min_percentile)
	...
	plt.plot(frame["x0"], linewidth=0.5)
	plt.ylabel('Pixels')
	plt.xlabel('Frames')
	plt.show()
